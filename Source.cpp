...
float lastTime = 0.0f; // ¬рем€ вывода последнего кадра
float deltaTime = 0.0f;//  ¬рем€, прошедшее между последним и текущим кадром
...
unsigned int loadTexture(char const* path)
{
    ...
    if (data)
    {
        GLenum format;  // nrComponents  количестве цветовых каналов получившегос€ изображени€
        if (nrComponents == 1)
            format = GL_RED;
        ...
        // создаем все необходимые мипмап-уровни д€ текстуры
        glGenerateMipmap(GL_TEXTURE_2D);
